...
public class ServerService extends IntentService {
	...
	/**
	 * Listens for changes in the server preferences, and immediately applies them to the server.  
	 */
	OnSharedPreferenceChangeListener prefListener_ = new OnSharedPreferenceChangeListener() {
		@Override
	    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
	    	// TODO: When android supports java 1.7 compliance, change this to 'switch'.
	    	
	    	// Check for the key that changed and make the appropriate change to the server
	...
	protected void onHandleIntent(Intent intent) {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(usbDescriptor_.getFileDescriptor())));
		BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(usbDescriptor_.getFileDescriptor())));
		...
		// TODO: is this actually necessary?  The only way to get here is to close the descriptor
	...
	BroadcastReceiver usbReceiver_ = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            if (usbAccessory_.equals(accessory)) {
            	try {
            		// Close the descriptor for our accessory (triggering shutdown)
            		usbDescriptor_.close();
            	} catch (IOException e) {
